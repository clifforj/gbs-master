 ...
 *  4. Generate C code + tile assets for compilation.
...
import { generateCode, generateTrampolines } from "./codegen.js";
...
// ── Resource bank ─────────────────────────────────────────────────────────────
// Large const data (font tiles, icon tiles, cover tiles) is placed in a
// dedicated ROM bank that does not overlap with the GBS music data.
...
/** Generate resource_bank.h with bank number and banked-ROM pointers. */
function generateResourceHeader(res: ResourceLayout): string {
  const hex = (n: number) => "0x" + n.toString(16).toUpperCase().padStart(4, "0");
  const base = 0x4000; // MBC1 banked window base address
  const lines = [
    ...
    "#include <stdint.h>",
    "",
    `#define RESOURCE_BANK       ${res.bank}`,
    "",
    "/* Pointers into the banked ROM window (0x4000-0x7FFF). */",
    "/* Select RESOURCE_BANK via MBC1 before dereferencing.  */",
    `#define RES_FONT_DATA       ((const uint8_t*)${hex(base + res.fontDataOff)}u)`,
    `#define RES_FONT_DATA_SIZE  ${res.fontDataSize}u`,
    `#define RES_FONT_WIDTHS     ((const uint8_t*)${hex(base + res.fontWidthsOff)}u)`,
    `#define RES_FONT_WIDTHS_SIZE ${res.fontWidthsSize}u`,
    `#define RES_SOFT_FONT       ((const uint8_t*)${hex(base + res.softFontOff)}u)`,
    `#define RES_SOFT_FONT_SIZE  ${res.softFontSize}u`,
    `#define RES_ICON_TILES      ((const uint8_t*)${hex(base + res.iconTilesOff)}u)`,
    `#define RES_ICON_TILES_SIZE ${res.iconTilesSize}u`,
    `#define RES_COVER_TILES     ((const uint8_t*)${hex(base + res.coverTilesOff)}u)`,
    `#define RES_COVER_TILES_SIZE ${res.coverTilesSize}u`,
    "",
...
export async function build(options: BuildOptions): Promise<void> {
  ...
  console.log("[3/6] Generating C code + assets...");
  await generateCode(gbs, tracks, ROM_GENERATED_DIR, { title: playlist?.title, wram, bankedCode });
  const assets = await generateAssets(options.coverPath);

  // Build the resource bank blob and generate the resource_bank.h header
  // BEFORE compilation so the C code can reference the bank/pointer defines.
  ...
  await writeFile(
    join(ROM_GENERATED_DIR, "resource_bank.h"),
    generateResourceHeader(res),
    "utf8"
  );
  console.log(`      Resource bank: ${res.bank} (${res.blob.length} bytes of tile data)`);
...
function compileCRom(gbdkHome: string, wram: WramLayout, bankedCode: BankedCodeOptions): void {
  ...
  // 1. Assemble startup.s (ROM skeleton) and generated trampolines.s.
  steps.push({
  ...
  //    --nostdlib:     no standard library — avoids RST vector conflicts with
  //                   GBDK's runtime and prevents __divuchar / __moduchar undefined
  //    --no-std-crt0: no default startup code — we provide our own startup.s
  //    -I src:        include path for header files (hardware.h, player.h, etc.)
  const cFiles = [
    "main", "player", "ui", "input",
    join("generated", "playlist_data"),
  ];
  ...
  const objs = ["startup", "trampolines", "main", "player", "ui", "input", "playlist_data"]
    .map((n) => `"${join(bd, n + ".rel")}"`)
    .join(" ");
  steps.push({
 ...
 * Bank 0 (startup + trampolines) always goes to 0x0000-0x3FFF.
...
function copyCompiledCode(
  ...
  // Always copy bank 0 (startup + trampolines).
...
async function embedGbs(
  ...
  gbsBytes.copy(rom, gbsRomOffset);
